...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const joinCall = async () => {
      try {
        ...
        setError(error instanceof Error ? error.message : "Error joining call");
      }
    };
    joinCall();
  ...
  if (error) {
    return (
      <StatusCard
        ...
        action={
          <button onClick={() => window.location.reload()} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
            Retry
          </button>
        }
      >
  ...
  if (!client) {
    return (
      <StatusCard title="Initializing client..." description="Setting up video call connection..." className="min-h-screen bg-blue-50">
